...
def get_quiz_questions():
    ...
    rows = pool.retry_operation_sync(callee)
    quiz_data = []
    for row in rows:
        # Проверяем тип options и преобразуем в список
        if isinstance(row.options, str):
            options = row.options.split(';')  # Разделяем строку на список по символу ';'
        else:
            options = row.options  # Если уже является списком
...
# Загрузка вопросов один раз при инициализации модуля
